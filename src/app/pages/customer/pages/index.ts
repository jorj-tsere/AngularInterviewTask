import { CustomerWrapperComponent } from '../customer-wrapper/customer-wrapper.component';
import { CustomerDetailsComponent } from './customer-details/customer-details.component';
import { CustomerListComponent } from './customer-list/customer-list.component';

// export * from './create-customer/create-customer.component';
// export * from './customer-list/customer-list.component';
// export * from './update-customer/update-customer.component';


export const customerPageComponents = [CustomerWrapperComponent, CustomerDetailsComponent, CustomerListComponent];
